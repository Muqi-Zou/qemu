...
//add up here since we may need this later
struct sgx_encl *encl = NULL;
struct sgx_encl *vm_private_data = NULL;

static int sgx_encl_create(struct sgx_secs *secs, struct sgx_encl *encl)
{
    ...
    */
	...
	//FIXME: for here, the encl is set/allocated but not used anywhere.
    //FIXED: we used a global variable to point this encl
    vm_private_data = encl;
    /*
    up_read(&current->mm->mmap_sem);
...
static abi_long do_ioctl_sgx_ioc_enclave_add_pages_in_kernel(const IOCTLEntry *ie, uint8_t *buf_temp,
                                     int fd, int cmd, abi_long arg)
{
    qemu_log_mask(LOG_UNIMP,
                      "In side do_ioctl_sgx_ioc_enclave_add_pages_in_kernel, we do not this function actually\n");
    if(DEBUG_SGX){
        printf("\n in do_ioctl_sgx_ioc_enclave_add_pages_in_kernel arg is %ld, cmd is %d \n", arg, cmd);
    }
    return (abi_long) 0;
}

//int sgx_encl_find(struct mm_struct *mm, unsigned long addr,struct vm_area_struct **vma)
static int sgx_encl_find(void)
{
...
//  For here, we can just point the return value to our global variable
    encl = vm_private_data;
//	*vma = result;
...
*/
	...
	if (!ret) {
        ...
        */
            if(DEBUG_SGX){
                printf("\n in the if !ret condition of sgx_get_encl");
            }
        }
	}
/*
...
static long sgx_ioc_enclave_add_page( unsigned int cmd,
				     unsigned long arg)
{
    ...
    if(DEBUG_SGX){
        printf("\nsecinfop is %ld(%08lx), secinfop void * & is %p,  secinfop void * is %p \n",secinfop,secinfop,(void *)&(secinfop),(void *)(secinfop) );
    }
	struct sgx_secinfo secinfo;
	struct sgx_encl *encl;
    ...
    */
    if (copy_from_user(&secinfo, (abi_ulong) secinfop,
			   (size_t) sizeof(secinfo))) {
        if(DEBUG_SGX){
            printf("\n things wrong inside the copy_from_user(&secinfo, (abi_ulong) secinfop,(size_t) sizeof(secinfo)))\n" );
        }           
		return -EFAULT;
	}else{
        if(DEBUG_SGX){
            printf("\n copy_from_user(&secinfo, (abi_ulong) secinfop,(size_t) sizeof(secinfo))) successfully!!\n" );
        }  
    }
...
static abi_long do_ioctl_sgx_ioc_enclave_add_page(const IOCTLEntry *ie, uint8_t *buf_temp,
                                     int fd, int cmd, abi_long arg)
{
    qemu_log_mask(LOG_UNIMP,
                      "In side do_ioctl_sgx_ioc_enclave_add_page, we do not this function actually\n");
    if(DEBUG_SGX){
        printf("\n in do_ioctl_sgx_ioc_enclave_add_page arg is %ld, cmd is %d \n", arg, cmd);
    }
    return (abi_long) sgx_ioc_enclave_add_page(cmd, arg);
}
